...
uniform float u_distort,u_contour,u_reveal;
...
void main(){
 ...
 float vs=shapeMask;
 vs*=bM(iC,.01);
 float diag=vP.x*.38+(1.-vP.y)*.62;
 float nR=sin(vP.x*17.+vP.y*13.+u_reveal*2.8)*.14
  +sin(vP.x*41.-vP.y*23.+u_reveal*1.6)*.09
  +sin((vP.x+vP.y)*8.2-u_reveal*3.4)*.11
  +pW(vec3(vP*8.4,u_reveal*2.1)).x*.16;
 vs*=1.-smoothstep(u_reveal-.3,u_reveal+.02,diag+nR);
...
export default function MetallicPaint({
  ...
  const revealEnabledRef = useRef(reveal);
  ...
  useEffect(() => {
    revealEnabledRef.current = reveal;
  }, [reveal]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      if (revealEnabledRef.current) {
        if (revealStartRef.current == null) revealStartRef.current = time + 180;
        const t = Math.min(
          1,
          Math.max(0, (time - revealStartRef.current) / revealDurationRef.current),
        );
        const eased = 1 - (1 - t) ** 2.55;
        const wander = Math.sin(t * Math.PI * 2.7) * 0.04 * Math.sin(t * Math.PI);
        revealAmt = (eased + wander) * 1.5;
      }
      gl.uniform1f(u.u_time, animTimeRef.current);
      gl.uniform1f(u.u_reveal, revealAmt);
